...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    if (!ssSetNumInputPorts(S, 8)) /* Number of input ports */
    {
        return;
    }
    ...
    ssSetInputPortWidth(S, 6, 3); // Target_XYZ (m)
    ssSetInputPortDirectFeedThrough(S, 6, 1);
    ssSetInputPortWidth(S, 7, 3); // Accel_command (Ax,Ay,Az) (m)
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    try
    {
        ...
        InputRealPtrsType target_xyz = ssGetInputPortRealSignalPtrs(S, 6);
        InputRealPtrsType accel_command = ssGetInputPortRealSignalPtrs(S, 7);
        ...
        if (availableLength)
        {
            ...
            ss << "\"target_xyz\":[" << (*target_xyz[0]) << "," << (*target_xyz[1]) << "," << (*target_xyz[2]) << "],";
            ss << "\"accel_command\":[" << (*accel_command[0]) << "," << (*accel_command[1]) << "," << (*accel_command[2]) << "]";
            ss << "}\n";
